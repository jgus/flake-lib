...
{ pkgs, source, pinSchema, branchOwnedFiles ? [ "pin.nix" "flake.lock" ], extraHashes ? [ ], versionOverrides ? { }, versionCanon ? [ ], excludePrereleases ? false, includePrereleaseAggregates ? false, minVersionComponents ? 3 }:
...
pkgs.writeShellApplication {
  ...
  runtimeInputs = pkgs.lib.optional (source.type == "pypi") (pkgs.python3.withPackages (pythonPackages: [ pythonPackages.packaging ]));
  runtimeEnv = {
    ...
    INCLUDE_PRERELEASE_AGGREGATES = pkgs.lib.optionalString includePrereleaseAggregates "1";
